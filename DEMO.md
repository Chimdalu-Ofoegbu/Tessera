...
5. **Trust close (~10s).** Note every metric carries its source + freshness. Mention the **public JSON API** (`/api/overview.json`) and that it's the real Renaiss card domain behind a swappable `DataSource` (`USE_RENAISS=1`).
